        ...
        while not self.stop_event.is_set():
            ...
            if previous_state_message_time is not None:
                ...
                if (
                    ...
                    > self.TIME_BETWEEN_STATE_MESSAGES_WARNING_THRESHOLD
                ):
                    print(
                        "Warning: Time between consecutive state messages is too long: ",
                        time_between_messages,
                    )
            ...
            if len(data) == STATE_MESSAGE_LENGTH:
                ...
                self.state = np.frombuffer(
                    data, dtype=StateMessageTypeAfterConfigurationChange
                )
...
RobotModeType = np.dtype(
    [
        (
            "packageSize",
            ">u4",
        ),  # >u4 means unsigned 4-byte integer in big-endian byte order
        (
            "packageType",
            ">u1",
        ),  # >u1 means unsigned 1-byte integer in big-endian byte order
        (
            "timestamp",
            ">u8",
        ),  # >u8 means unsigned 8-byte integer in big-endian byte order
        ...
        (
            "targetSpeedFraction",
            ">f8",
        ),  # >f8 means 8-byte float in big-endian byte order
        ("speedScaling", ">f8"),
            ...
            np.dtype(
                [
                    ...
                    (
                        "I_actual",
                        ">f4",
                    ),  # >f4 means 4-byte float in big-endian byte order
                    ("V_actual", ">f4"),
                    ("T_motor", ">f4"),
                    ("T_micro", ">f4"),
                    ("jointMode", ">u1"),
...
ConfigurationDataType = np.dtype(
    [
        ...
        (
            "masterboardVersion",
            ">i4",
        ),  # >i4 means 4-byte integer in big-endian byte order
