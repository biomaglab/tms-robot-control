...
class MultiAxisImpedancePIDController:
    def __init__(self, P=0.3, I=0.01, D=0.0, stiffness=0.05, damping=0.02, mode='impedance'):
        self.Kp = np.array(P) if isinstance(P, (list, tuple, np.ndarray)) else np.array([P]*3)
        self.Ki = np.array(I) if isinstance(I, (list, tuple, np.ndarray)) else np.array([I]*3)
        self.Kd = np.array(D) if isinstance(D, (list, tuple, np.ndarray)) else np.array([D]*3)
        ...
        # Setpoints
        self.displacement_setpoint = 0.0  # Z only
        self.force_setpoint = np.array([-5.0, 0.0, 0.0])  # [Fz, Tx, Ty]
        ...
        self.output_min = -np.ones(3) * np.inf
        self.output_max = np.ones(3) * np.inf
        self.enabled = True

    def update(self, feedback_z, force_feedback_vec):
        if not self.enabled:
            return self.output

        self.current_time = time.monotonic()
        delta_time = self.current_time - self.last_time
        if self.sample_time > 0 and delta_time < self.sample_time:
            return self.output

        # Compute errors
        displacement_error = self.displacement_setpoint - feedback_z
        force_error = self.force_setpoint - np.array(force_feedback_vec)

        # PID terms for [Fz, Tx, Ty]
        PTerm = self.Kp * force_error
        self.ITerm += force_error * delta_time
        self.ITerm = np.clip(self.ITerm, -self.windup_guard, self.windup_guard)
        DTerm = (force_error - self.last_force_error) / delta_time if delta_time > 0 else np.zeros(3)

        # Impedance effect (for Z only)
        impedance_z = self.stiffness * displacement_error - self.damping * self.velocity[0]
        control_z = PTerm[0] + self.Ki[0] * self.ITerm[0] + self.Kd[0] * DTerm[0] + impedance_z

        # Update Z velocity
        acc_z = control_z - self.damping * self.velocity[0]
        self.velocity[0] += acc_z * delta_time

        # Compute pitch and roll outputs from Tx and Ty PID
        pitch = PTerm[1] + self.Ki[1] * self.ITerm[1] + self.Kd[1] * DTerm[1]
        roll = PTerm[2] + self.Ki[2] * self.ITerm[2] + self.Kd[2] * DTerm[2]

        self.output = np.clip(
            np.array([control_z, pitch, roll]),
            self.output_min,
            self.output_max
        )
        ...
        return self.output  # [Δz, Δpitch, Δroll]

    # --- Config ---
    def set_force_setpoint(self, fz, tx, ty):
        self.force_setpoint = np.array([fz, tx, ty])

    def set_pid_gains(self, P, I, D):
        self.Kp = np.array(P)
        self.Ki = np.array(I)
        self.Kd = np.array(D)

    def set_output_limits(self, min_vals, max_vals):
        self.output_min = np.array(min_vals)
        self.output_max = np.array(max_vals)
