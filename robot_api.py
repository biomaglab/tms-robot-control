...
class RobotApi:
    ...
    def update_robot_status(self, success):
        if self.connection is not None:
            self.connection.update_robot_status(success=success)

    def robot_connection_status(self, success):
        if self.connection is not None:
            self.connection.robot_connection_status(success=success)

    def robot_pose_collected(self, success):
        if self.connection is not None:
            self.connection.robot_pose_collected(success=success)

    def set_objective(self, objective):
        if self.connection is not None:
            self.connection.set_objective(objective=objective)
    ...
    def __set_callbacks(self, connection):
        ...
        connection.set_callback__reset_robot_transformation_matrix(
            self.on_reset_robot_matrix
        )
        connection.set_callback__estimate_robot_transformation_matrix(
            self.on_robot_matrix_estimation
        )
        connection.set_callback__set_robot_transformation_matrix(
            self.on_set_robot_transformation_matrix
        )
        connection.set_callback__update_displacement_to_target(
    ...
    def on_set_tracker_fiducials(self, fiducial_left, fiducial_right, fiducial_nasion):
        if self.connection is not None:
            m_fiducial_left = (
                np.array(fiducial_left, dtype=float).reshape((4, 4)).tolist()
            )
            m_fiducial_right = (
                np.array(fiducial_right, dtype=float).reshape((4, 4)).tolist()
            )
            m_fiducial_nasion = (
                np.array(fiducial_nasion, dtype=float).reshape((4, 4)).tolist()
            )
            data = {
                "tracker_fiducials": [
                    m_fiducial_left,
