...
import csv
file = open("tracker_coord.csv")
csvreader = csv.reader(file, quoting=csv.QUOTE_NONNUMERIC)
header = next(csvreader)
tracker_coord = []
for row in csvreader:
    tracker_coord.append(row)
file.close()
file = open("tracker_angles.csv")
csvreader = csv.reader(file, quoting=csv.QUOTE_NONNUMERIC)
header = next(csvreader)
tracker_angles = []
for row in csvreader:
    tracker_angles.append(row)
file.close()
file = open("robot_coord.csv")
csvreader = csv.reader(file, quoting=csv.QUOTE_NONNUMERIC)
header = next(csvreader)
robot_coord = []
for row in csvreader:
    robot_coord.append(row)
file.close()
file = open("robot_angles.csv")
csvreader = csv.reader(file, quoting=csv.QUOTE_NONNUMERIC)
header = next(csvreader)
robot_angles = []
for row in csvreader:
    robot_angles.append(row)
file.close()

coord_coil_list = np.zeros((4, 4))[np.newaxis]
robot_coord_list = np.zeros((4, 4))[np.newaxis]

for i in range(len(tracker_coord)):
    new_coord_coil_list = np.array(elfin_process.coordinates_to_transformation_matrix(
        position=tracker_coord[i],
        orientation=tracker_angles[i],
        axes='rzyx',
    ))

    coord_coil_list = np.vstack([coord_coil_list.copy(), new_coord_coil_list[np.newaxis]])

for i in range(len(tracker_coord)):
    new_robot_coord_list = elfin_process.coordinates_to_transformation_matrix(
        position=robot_coord[i],
        orientation=robot_angles[i],
        axes='rzyx',
    )
    robot_coord_list = np.vstack([robot_coord_list.copy(), new_robot_coord_list[np.newaxis]])
